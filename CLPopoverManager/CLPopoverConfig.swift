...
@objcMembers public class CLPopoverConfig: NSObject {
    ...
    public enum CLDisplayMode {
        /// 排队，如果没有正在显示，会立即显示，如果有正在显示，会加入到同分组等待队列，后续按照优先级显示
        case queue
        /// 插队，立即展示，可能会重叠
        case interrupt
        /// 挂起同分组当前显示的所有弹窗，并显示自己，当自己消失后，恢复被挂起的弹窗
        case suspend
        /// 替换同分组当前正在显示的弹窗，并继承其挂起链，不会移除同分组等待中的弹窗
        case replaceInheritSuspend
        /// 替换同分组当前正在显示的弹窗，并清除其挂起链，不会移除同分组等待中的弹窗
        case replaceClearSuspend
        /// 替换同分组当前正在显示的弹窗，会移除同分组被挂起的弹窗，也会移除同分组等待中的弹窗
        case replaceAll
        /// 唯一，替换同分组当前正在显示的弹窗，会移除同分组被挂起的弹窗，也会移除同分组等待中的弹窗，会阻止后续同分组弹窗
        case groupUnique
        /// 全局唯一，替换所有分组正在显示的弹窗，会移除所有分组被挂起的弹窗，也会移除所有分组等待中的弹窗，会阻止后续所有弹窗
        case globalUnique
    }

    /// 弹窗的唯一标识符，全局去重，不受groupID影响，如果需要按照group分组，自己用groupID_XXX字符串实现
    public var identifier: String?
    /// 分组ID，相同分组的弹窗在一起调度，不同分组互不影响，nil表示默认分组
